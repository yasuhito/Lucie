...
class ShellTest < Test::Unit::TestCase
  def test_on_exit_block_should_be_called
    process = Object.new
    process.stubs( :popen3 )
    process.stubs( :wait )
    Popen3::Popen3.expects( :new ).returns( process )
  ...
  def test_puts
    ...
    process = Object.new
    process.stubs( :popen3 ).yields( tochild_mock, mock( 'FROMCHILD', :gets => nil ), mock( 'CHILDERR', :gets => nil ) )
    process.stubs( :wait )
    Popen3::Popen3.stubs( :new ).returns( process )
  ...
  def test_on_stdout
    ...
    process.stubs( :popen3 ).yields( nil, fromchild_mock, mock( 'CHILDERR', :gets => nil ) )
    process.stubs( :wait )
    Popen3::Popen3.stubs( :new ).returns( process )
  ...
  def test_on_stderr
    ...
    process = Object.new
    process.stubs( :popen3 ).yields( nil, mock( 'FROMCHILD', :gets => nil ), childerr_mock )
    process.stubs( :wait )
    Popen3::Popen3.stubs( :new ).returns( process )
  ...
  def test_sh_exec___fail___
    ...
    assert_raises( 'Command "TEST_COMMAND TEST_ARG1 TEST_ARG2" failed' ) do
      Kernel.sh_exec( 'TEST_COMMAND', 'TEST_ARG1', 'TEST_ARG2' )
    end
  end
