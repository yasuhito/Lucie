...
describe SuperReboot do
  before :each do | each |
    ...
    @debug_option = { :dry_run => false, :verbose => true }
    @messenger = StringIO.new( "" )
    @super_reboot = SuperReboot.new( nil, @debug_option, @messenger )
  ...
  context "when failed to reboot with script" do
    it "should fall back to ssh reboot" do
      ...
      @super_reboot.stub( :run ).with( "false yutaro", @debug_option, @messenger ).and_raise( RuntimeError )
      @super_reboot.stub( :run ).with( /\Assh.*root@yutaro "reboot"\Z/, @debug_option, @messenger )
  ...
  context "when failed to super reboot" do
    it "should raise" do
      logger = mock( "logger" )
      @super_reboot.stub( :run ).with( "false yutaro", @debug_option, @messenger ).and_raise( RuntimeError )
      @super_reboot.stub( :run ).with( /\Assh.*root@yutaro "reboot"\Z/, @debug_option, @messenger ).and_raise( RuntimeError )
      lambda do
