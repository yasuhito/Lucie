...
機能: SSH のキーペアの自動生成と認証
  ...
  テンプレ: SSH キーペアの自動生成
    前提 ホームディレクトリは "/tmp/yasuhito"
    かつ ホームディレクトリに SSH のキーペアが "<SSH のキーペア>"
    かつ ホームディレクトリに Lucie 用の SSH キーペアが "<Lucie 用のキーペア>"
    もし SSH のキーペアを生成し、認証しようとした
    ならば SSH のキーペアは "<キーペアの生成>"

  サンプル:
    | SSH のキーペア | Lucie 用のキーペア | キーペアの生成 |
    | 存在する       | 存在する          | 生成されない   |
    | 存在しない     | 存在する          | 生成されない   |
    | 存在する       | 存在しない        | 生成されない   |
    | 存在しない     | 存在しない        | 生成される     |
  ...
  シナリオ: authorized_keys が無い場合、公開鍵をコピー
    前提 ホームディレクトリは "/tmp/yasuhito"
    かつ ホームディレクトリに SSH のキーペアがすでに存在
    かつ ホームディレクトリに Lucie 用の SSH キーペアが存在しない
