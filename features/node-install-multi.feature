Feature: node install-multi command
  ...
  Scenario: node install-multi
    ...
    When I try to run 'node install-multi' with global option "--netmask 255.255.255.0"
    Then node "yasuhito_node0" installed
    And node "yasuhito_node1" installed
    And node "yasuhito_node2" installed
  ...
  Scenario: node local --storage-conf option
    ...
    And install option for node "yasuhito_node2" is "--ip-address 192.168.0.102 -mac 11:22:33:44:55:22 --storage-conf storage2.conf"
    When I try to run 'node install-multi' with global option "--storage-conf global_storage.conf --netmask 255.255.255.0"
    Then node "yasuhito_node0" installed using storage conf "storage0.conf"
    And node "yasuhito_node1" installed using storage conf "global_storage.conf"
    And node "yasuhito_node2" installed using storage conf "storage2.conf"
  ...
  Scenario: global --storage-conf option
    ...
    When I try to run 'node install-multi' with global option "--storage-conf global_storage.conf --netmask 255.255.255.0"
    Then node "yasuhito_node0" installed using storage conf "global_storage.conf"
    And node "yasuhito_node1" installed using storage conf "global_storage.conf"
    And node "yasuhito_node2" installed using storage conf "global_storage.conf"
