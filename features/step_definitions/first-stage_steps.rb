When /^I try to setup first stage environment for node "([^\"]*)"$/ do | name |
  ...
  first_stage.start [ Nodes.find( name ) ], Installer.new, [ @if ]
end
...
When /^I run first stage installer with node "([^\"]*)"$/ do | name |
  ...
  first_stage = FirstStage.new( Nodes.find( name ), "linux-image-686", "./my_storage.conf", "LDB_DIRECTORY", logger, html_logger, { :dry_run => true }, @messenger )
  first_stage.run
end
