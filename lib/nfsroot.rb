...
class Nfsroot < Rake::TaskLib
    ...
    namespace 'installer' do
      ...
      task paste( 'clobber_', @name ) do
        if File.exist?( @target_directory )
          puts "#{ @target_directory } already exists. Removing #{ @target_directory }"
  ...
  def hoaks_packages
    puts 'Modifying nfsroot to avoid errors caused by some packages.'
  ...
  def add_packages_nfsroot
    ...
    packages = ( [ 'ruby', 'reiserfsprogs', 'discover', 'module-init-tools', 'ssh', 'udev', 'console-tools', 'psmisc', 'file', 'ganglia-monitor' ] << @extra_packages ).flatten.uniq.compact
    puts "Adding packages to nfsroot: #{ packages.join( ', ' ) }"
    puts "NOTE: Error outputs generated by package installation scripts can be safely ignored."
  ...
  def install_kernel_nfsroot
    ...
    puts "Kernel #{ get_kernel_version } installed into the nfsroot."
  ...
  def setup_dhcp
    ...
    puts "Kernel #{ get_kernel_version } copied to #{ tftp_kernel_target }"
    sh_exec "cp #{ pxebin } /srv/tftp/lucie/"
    puts "DHCP environment prepared. If you want to use it, you have to enable the dhcpd and the tftp-hpa daemon."
  end


  def finish_nfsroot
    File.open( target( '/etc/gmond.conf' ), 'w' ) do | f |
      f.puts( <<-EOF )
...
EOF
    ...
    sh_exec %{echo "* * * * * root /usr/bin/gmetric -nlucie_status -vOK -tstring" > #{ target( '/etc/crontab' ) }}
...
case $prog in
    *cron) /sbin/start-stop-daemon.distrib --start --quiet --pidfile /var/run/crond.pid --name cron --startas /usr/sbin/cron --
           ;;
    *gmond) /sbin/start-stop-daemon.distrib --start --quiet -m --pidfile /var/run/gmond.pid --exec /usr/sbin/gmond
           ;;
