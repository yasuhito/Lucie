...
task :tidy do
  ...
  scripts = FileList[ File.join( RAILS_ROOT, 'script/**/*' ) ]

  ( mvc + specs + stories + builder_plugins + libs + scripts ).each do | each |
    if File.file?( each ) and IO.read( each ).grep( /utf-8-unix/ ).empty?
      list << each
    end
  end
  unless list.empty?
