class Scm
  class Git < Common
    def clone url, target
      run "git clone #{ url } #{ target }", { "GIT_SSH" => "ssh -i #{ private_key_path }" }
    end
    ...
    def update target
      test_installed
      run "cd #{ target } && git pull", { "GIT_SSH" => "ssh -i #{ private_key_path }" }
      run "cd #{ target } && git update", { "GIT_SSH" => "ssh -i #{ private_key_path }" }
    end
