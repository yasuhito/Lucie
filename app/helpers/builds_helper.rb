module BuildsHelper
  ...
  def builds_except_last(installer, n)
    installer.builds.reverse[n..-1]
  end
  ...
  def link_to_code(log)
    ...
    log.gsub(/(\#\{RAILS_ROOT\}\/)?([\w\.-]*\/[ \w\/\.-]+)\:(\d+)/) do |match|
      ...
      if path.index(@work_path) == 0
        ...
        link_to match, "/installers/code/#{h @installer.name}#{path}?line=#{line}##{line}"
      else
        match
      end
    end
  ...
  def format_installer_settings(settings)
    settings = settings.strip
    if settings.empty?
      "This installer has no custom configuration. Maybe it doesn't need it.<br/>" +
